...
def get_avg_resolution(img_lists) -> tuple[tuple[int, int], tuple[str, str]]:
...
def thread_launcher(img_lists, scale, thread_num):
    ...
    with ThreadPoolExecutor(thread_num) as f:
        for img in img_lists:
            time.sleep(0.1)
            f.submit(processor, img, scale, pbar)


def main(workdir, scale: int = 2, thread_num: int = 10):
    img_lists = get_images(workdir)
    avg_resolution = get_avg_resolution(img_lists)
    print(f"平均像素{avg_resolution[0]} {avg_resolution[1][0]}")
    if avg_resolution[1][1] == "Low" or avg_resolution[1][1] == "Middle":
        user_input = input(
            f"是否需要ai优化图片,预计耗时{int(len(img_lists) / 60)}min (Y|n)>>>"
        )
        if user_input.replace(" ", "") not in ["y", "", "Y"]:
            return
    elif avg_resolution[1][1] == "High":
        user_input = input(
            f"是否需要ai优化图片,预计耗时{int(len(img_lists) / 60)}min (N|y)>>>"
        )
        if user_input.replace(" ", "") in ["n", "", "N"]:
            return

    thread_launcher(img_lists, scale, 4)
